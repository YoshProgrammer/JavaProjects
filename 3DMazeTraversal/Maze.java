...
public class Maze
{
    ...
    private int numberRows, numberColumns, numberCells;
    private int[][][] grid;
    ...
    public Maze(String filename) throws FileNotFoundException
    {
        ...
        numberCells = scan.nextInt();
        
        grid = new int[numberRows][numberColumns][numberCells];
     ...
     */
    public void tryPosition(int row, int col, int cell)
    {
        grid[row][col][cell] = TRIED;
    }
     ...
     */
    public void markPath(int row, int col, int cell)
    {
     ...
     */
    public boolean validPosition(int row, int column, int cell)
    {
        boolean result = false;
 
        // check if cell is in the bounds of the matrix 
        if (row >= 0 && row < grid.length &&
            column >= 0 && column < grid[row].length && cell >= 0 && cell < grid[row][column].length) {

            //  check if cell is not blocked and not previously tried 
            if (grid[row][column][cell] == 1) {
                result = true;
            }
        }
    ...
    public String toString()
    {
        ...
        for (int row=0; row < grid.length; row++)
        {
            for (int column=0; column < grid[row].length; column++) {
                for (int cell = 0; cell < grid[row][column].length; cell++) {
                    result += grid[row][column][cell] + "";
                }
                result += "\n";
            }
            result += "\n";
